...
class PCSF_run_class(MySQLConnect_class, object):
    def __init__(self, PIUMet_input, w, b, D, mu, DummyConnection, msgpath):
        ...
        # first creates a temp file that will have interactome undirected edges (E), nodes with prizes (W), and root (R)
        # additionally, mz-Matched met will be added, as well as Dummy connections
        ...
        msg_input = self.make_temp_msg_inputFile(PIUMet_input.interactome.net, PIUMet_input.mzMet_net_union,
                                                 all_terminals, all_terminals_MCP_id, w, b, mu)
        ...
        # querying from Mysql table so it won't run out.
        sql = "select 1"
    ...
    def msg_run(self, msg_input, D, msgpath):
        ...
        msg_args = [msgpath, '-d', str(D), '-t', '1000000', '-o', '-r', '1e-5', '-g', '1e-3']
        msg_input.seek(0)  # going to the beginning of the file
        self.inputFile = msg_input.readlines()
        msg_input.seek(0)

        print "-running msg optimization"
