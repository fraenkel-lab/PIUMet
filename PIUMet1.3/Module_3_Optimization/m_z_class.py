# this code fdefines a class for m/z metabolite
#
#
# by: Leila Pirhaji
# ---------------------------
...
# this class will obtain a mz value, and return matched metabolite
class m_z_class(MySQLConnect_class, object):
    ...
    def __init__(self, mz, mode, prize, superClass, ppm, mzW, interactome):
        ...
        # -----------------
        # calculating mz, and correcting by the mode of the detection
        if mode == 'positive':
            self.mz = float(mz) - 1
        elif mode == "negative":
            self.mz = float(mz) + 1
        ...
        # ---------------------
        # creating an arbitarary MCP ID for each peak that start with "m/z=":
        self.MCP_id = "m/z=" + str(self.mz)

        # ---------------------
        # identifying the matched metabolites to m/z
        self.find_mz_matched_met(ppm)

        # ----------------------
        # making a network representation of matched metabolites and m/z
        # this is only for the metabolites present at the network
    ...
    def find_mz_matched_met(self, ppm):
        ...
        sql_cond1 = "where ((%s - %s < %s ) and (%s - %s > %s ))" % (
        col_mass, self.mz, ppm, col_mass, self.mz, str(-1 * float(ppm)))

        # -------
        # filtering the select by the metabolite origin to include only food and Endogenous metabolites
        sql_cond2 = "and (origin like '%food%' or origin like '%Endogenous%' or origin is NULL)"

        # -------
        # filtering based on the super class
        if self.superClass == 'Lipids':
            sql_cond3 = "and (super_class='Lipids' or super_class is NULL)"
            sql_cond = sql_cond1 + " " + sql_cond2 + " " + sql_cond3
        else:
    ...
    # this function creates a network of m/z matched metabolites
