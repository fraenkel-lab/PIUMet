# this is the main code for running PIUMet
#
#
# by: Leila Pirhaji
# ----------------------
...
# ---------------------------------------
# My own packages
sys.path.append('/home/narek/PyCharmProjects//PIUMet1.3')
...
def writing_output_summary_file(result_path, parameters_dic, PIUMet_input, PIUMet_union_result):
    # writing a summary file, including 1.# m/z 2.# mathced metabolites 3. #matched metabolites in the interactome,
    # 4. size of the network 5. summary of parameters
    ...
    f.write(('w=%s ,D=%s, mu=%s, beta=%s, mzW=%s, Da=%s \n') % (
        parameters_dic['w'], parameters_dic['D'], parameters_dic['mu'], parameters_dic['b'], parameters_dic['mzW'],
        parameters_dic['Da']))
...
# this is the main function that calls all the classes and functions and gets the input parameteres
def main():
    # inputs:
    parser = OptionParser()

    # parser.add_option("-I", "--interactome", help="the path to the interactome", default="/home/lpirhaji/PIUMet/HMDB_Recon_iRef_net.txt")
    parser.add_option("-I", "--interactome", help="the underlying database",
                      default="/home/lpirhaji/PIUMet/PIUMet/PIUMet1.1/HMDB_Recon_iRef_net_woLoop_woSpace.pkl")
    parser.add_option("-t", "--TerminalFile",
                      ...
                      default="/home/lpirhaji/ADomics_projects/data/peak_terminal_file.txt")
    parser.add_option("--optionalTerminalFile",
                      help=" tab delimited input file, including termonal protein and metabolite files, that indicate thier their type (i.e. protein or metabolite) and their prizes as 'TerminalName \t terminalPrize \t terminalType'",
                      default=None)
    parser.add_option("-p", "--parameteres",
    ...
    parser.add_option("-o", "--outdirectory", help="the path to save the results",
                      default="/home/lpirhaji/ADomics_projects")
    parser.add_option("--msgPath", help="the path message pathing code", default="/nfs/apps/bin/msgsteiner9")  # algorithm
    parser.add_option("--DummyConnection",
                      help="the mode of dummy connections, from the following: 'All', 'Terminals', list of nodes as a text file, eahc line contains a node",
                      default="Terminals")
    parser.add_option("--detectedMetClass",
    ...
    # --------------------------------
    # creating input object for running PCSF
    ...
    PIUMet_PCSFout = PCSF_run_class(PIUMet_input, float(parameters_dic['w']), float(parameters_dic['b']),
                                    int(parameters_dic['D']), float(parameters_dic['mu']), dummyCon, msgpath)
    ...
    if int(R) != 0:
        for i in range(1, int(R) + 1):
            ...
            PIUMet_PCSFout_rand = PCSF_run_class(PIUMet_input_rand, float(parameters_dic['w']),
                                                 float(parameters_dic['b']), int(parameters_dic['D']),
...
# Calls writing_output_summary_file() for each w, b, and R
def writing_output_summary_files(result_path, parameters_dic, PIUMet_input, PIUMet_union_result, omegas, be$
        """
        Creates summary files for all parameters
        """
        total_nodes = []
        total_HMDB_peaks = []
        total_PPMI_peaks = []
        for w in omegas:
                for b in betas:
                        for R in numRepeats:
                                nodes, peaks_HMDB, peaks_PPMI = writing_output_summary_file(result_path, w,$
                                total_nodes.append(nodes)
                                total_HMDB_peaks.append(peaks_HMDB)
                                total_PPMI_peaks.append(peaks_PPMI)
        return total_nodes, total_HMDB_peaks, total_PPMI_peaks