...
#My functions
def writing_output_summary_file(result_path, parameters_dic, PIUMet_input, PIUMet_union_result, numRepeats):
	...
	if result_path[:-1]=="/":
		f=open(result_path+"result_summary_%s.txt" %numRepeats,'w')
	else:
		f=open(result_path+"/"+"result_summary_%s.txt" %numRepeats,'w')
...
def main():
	...
	parser.add_option("-I", "--interactome", help="the underlying database", default=INTERACTOME) #"/home/lpirhaji/PIUMet/PIUMet/PIUMet1.1/HMDB_Recon_iRef_net_woLoop_woSpace.pkl")
	...
	parser.add_option("--optionalTerminalFile", help=" tab delimited input file, including termonal protein and metabolite files, that indicate their type (i.e. protein or metabolite) and their prizes as 'TerminalName \t terminalPrize \t terminalType'",
			 default=None)	
	parser.add_option("-p", "--parameters", help="a file containing the paramters of the model, including w,b,D,mu,mzW,Da as 'parameteres_name = parametere_value'",
			 default="/home/lpirhaji/ADomics_projects/data/parameteres_w15mu015.txt")
	parser.add_option("-o", "--outdirectory", help="the path to save the results", default=OUTDIRECTORY) #"/home/lpirhaji/ADomics_projects")
	parser.add_option("--msgPath", help="the path message pathing code", default="/nfs/apps/bin/msgsteiner9")
	parser.add_option("--DummyConnection", help="the mode of dummy connections, from the following: 'All', 'Terminals', list of nodes as a text file, each line contains a node",
			 default="Terminals")	
	parser.add_option("--detectedMetClass", help="the class of metabolites detectable by experiments: 1.Lipids 2.others; multiple options can be selcted, and write with comma",
	...
	PIUMet_union_result = resulting_union_net_class(PIUMet_PCSFout, PIUMet_PCSFout_rand_list, result_path, R)
	#---------------------------------
	...
	writing_output_summary_file(result_path, parameters_dic, PIUMet_input, PIUMet_union_result, R)
