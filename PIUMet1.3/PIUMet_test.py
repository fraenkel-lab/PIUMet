...
def writing_output_summary_file(result_path, parameters_dic, PIUMet_input, PIUMet_union_result, w, b, R):
	...
	#-----------------------------------------------------------------------------------
	nMZ, nMZ_all_deg, nMZ_ppmi, nMZ_result, nNode_tree, nEdge_tree, nMZ_all = calculate_statistics(PIUMet_input, PIUMet_union_result)
	#----------------
	#writing the info about m/z values.
	#nMZ=0
	#nMZ_all_deg=0
...
def main():
	...
	parser.add_argument("-I", "--interactome", help="the underlying database", default=INTERACTOME) #"/home/lpirhaji/PIUMet/PIUMet/PIUMet1.1/HMDB_Recon_iRef_net_woLoop_woSpace.pkl")
	parser.add_argument("-t", "--TerminalFile", help=" tab delimited input file, including the m/z values and the mode of LC; a text file in which each line has to contain one mass and mode",
			 default=TERMINAL) #"/home/lpirhaji/ADomics_projects/data/peak_terminal_file.txt")
	parser.add_argument("--optionalTerminalFile", help = " tab delimited input file, including termonal protein and metabolite files, that indicate their type (i.e. protein or metabolite) and their prizes as 'TerminalName \t terminalPrize \t terminalType'",
			 default=None)	
	parser.add_argument("-p", "--parameters", help = "a file containing the paramters of the model, including w,b,D,mu,mzW,Da as 'parameteres_name = parametere_value'",
	...
	parser.add_argument("-b", "--betas", type=parse_range, help = "a list of beta values to run the algorithm",
			 default="4")
	parser.add_argument("-o", "--outdirectory", help="the path to save the results", default=OUTDIRECTORY) #"/home/lpirhaji/ADomics_projects")
	parser.add_argument("--msgPath", help="the path message pathing code", default="/nfs/apps/bin/msgsteiner9")
	parser.add_argument("--DummyConnection", help="the mode of dummy connections, from the following: 'All', 'Terminals', list of nodes as a text file, each line contains a node",
			 default="Terminals")	
	parser.add_argument("--detectedMetClass", help="the class of metabolites detectable by experiments: 1.Lipids 2.others; multiple options can be selcted, and write with comma",
			 default="Lipids,others")
	parser.add_argument("-R", "--numRepeats", type=parse_range, help="a list of number of repeatsto run the algorithm on",
			 default="0")
        ...
        if int(R) != 0:
        	for i in range(1,int(R)+1):
                	print ("randomization number %s:" %str(i))
                        ...
                        PIUMet_PCSFout_rand = PCSF_run_class(PIUMet_input_rand,float(w),float(b),int(parameters_dic['D']), float(parameters_dic['mu']), dummyCon,msgpath)
