# This class generates a resulting union network with statistical information
# about nodes and edges and performs an ID conversion
#
#
#by: Leila Pirhaji
#----------------------------
...
class resulting_union_net_class(MySQLConnect_class,object):
    ...
    def __init__ (self, PIUMet_PCSFout, PCSF_out_list, result_path):
        #-----------------------
        #1. obtaining union network of all the resulting nets
        ...
        #-----------------------
        #2. calculating the frequency of nodes and edges in the resulting nets, save as node and edge attributes
        ...
        self.result_2_textfile(result_path)
    ...
    def ID_conversion_MCPid_2_commonName(self):
        #this function will call an appropriate ID conversion function based on the node Type
        self.conn.ping(True)
    ...
    #-----------------------------------
    def result_2_textfile(self, result_path):
        print (result_path)
        if result_path[-1:] == "/":
            f_net = open(result_path + "result_union_net.txt", 'w')
            f_node_frq = open(result_path + "result_node_frequency.txt", 'w')
            f_edge_frq = open(result_path + "result_edge_frequency.txt", 'w')
        else:
            f_net = open(result_path + "/result_union_net.txt", 'w')
            f_node_frq = open(result_path + "/result_node_frequency.txt", 'w')
            f_edge_frq = open(result_path + "/result_edge_frequency.txt", 'w')
...
def result_2_textfiles(self, result_path, omegas, betas, numRepeats):
    print result_path
    for w in omegas:
        for b in betas:
            print "Generating results for w = %d and b = %d " % (w, b)
            for R in numRepeats:
