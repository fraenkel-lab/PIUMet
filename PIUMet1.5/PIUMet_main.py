...
def main():
    ...
    parser.add_option("-t", "--TerminalFile",
                      ...
                      default="/home/lpirhaji/ADomics_projects/data/peaks_terminal.txt")
    ...
    parser.add_option("-o", "--outdirectory", help="the path to save the results",
                      default="/home/lpirhaji/ADomics_projects/result_MST")
    parser.add_option("--msgPath", help="the path message pathing code",
                      default="/home/lpirhaji/PIUMet/PIUMet/PIUMet1.4/mst_code/mst_clustering_singlecore")
    ...
    parser.add_option("-w", "--DummyW",
                      help="Tuning parameter, the weight of Dummy node connections as start,end,increment",
                      default="9,9,0")
    parser.add_option("--beta",
                      help="Tuning parameter, the relative weight of node prize to edge cost, as start,end,increment",
                      default="4,4,0")
    parser.add_option("--mu",
                      help="Tuning parametertuning parameter that controls the effect negative prizes, as start,end,increment",
                      default="0.001,0.01,0.001")
    parser.add_option("--Da", help="The thershold between observed mass and actual mass of metabolites", default="0.05")
    ...
    for wi in w:
        for betai in beta:
            for mui in mu:
                ...
                print ("Running PIUMet for the following parameters: w=%s, beta=%s, mu=%s for R=%s repeats" % (
                wi, betai, mui, R))
                PIUMet_result_set.update({parameter_key: PIUMet_result_1_set_parameter(PIUMet_input, float(wi),
                                                                                       float(betai), float(mui),
                                                                                       dummyCon, msgpath, result_path,
                                                                                       float(Da), R)})
