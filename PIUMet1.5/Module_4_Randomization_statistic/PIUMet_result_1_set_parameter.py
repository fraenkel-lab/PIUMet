# this code will provide a PIUMet output for one set of w, beta and mu parameter
#
#
# by:Leila Pirhaji
# ----------------------------------
...
class PIUMet_result_1_set_parameter(object):
    ...
    def __init__(self, PIUMet_input, w, beta, mu, dummyCon, msgpath, result_path, Da, R):
        ...
        self.PIUMet_union_result = resulting_union_net_class(self.PIUMet_PCSFout, self.PIUMet_PCSFout_rand_list,
                                                             result_path, w, beta, mu)

        # --------------------------------------
        # writing the output summary file
        self.writing_output_summary_file(result_path, w, beta, mu, Da, PIUMet_input)

    # --------------------------------------------
    # this function will write a summary of the output files
    def writing_output_summary_file(self, result_path, w, beta, mu, Da, PIUMet_input):
        ...
        if result_path[:-1] == "/":
            f = open(result_path + ("result_summary_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
        else:
            f = open(result_path + "/" + ("result_summary_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
        f.write('Summary of the results, for the following parameters:\n')
        f.write(('w=%s , beta=%s, mu=%s, Da=%s \n') % (w, beta, mu, Da))
