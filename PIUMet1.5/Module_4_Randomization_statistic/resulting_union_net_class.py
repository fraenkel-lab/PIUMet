# this class genrate a resulting union network
...
class resulting_union_net_class(MySQLConnect_class, object):
    def __init__(self, PIUMet_PCSFout, PCSF_out_list, result_path, w, beta, mu):
        # -----------------------
        ...
        self.result_2_textfile(result_path, str(w), str(beta), str(mu))
    ...
    # -----------------------------------
    def result_2_textfile(self, result_path, w, beta, mu):
        print (result_path)
        if result_path[-1:] == "/":
            f_net = open(result_path + ("result_union_net_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
            f_node_frq = open(result_path + ("result_node_frequency_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
            f_edge_frq = open(result_path + ("result_edge_frequency_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
        else:
            f_net = open(result_path + "/" + ("result_union_net_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
            f_node_frq = open(result_path + "/" + ("result_node_frequency_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
            f_edge_frq = open(result_path + "/" + ("result_edge_frequency_w%s_b%s_mu%s.txt" % (w, beta, mu)), 'w')
