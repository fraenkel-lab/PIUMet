...
class resulting_union_net_class(MySQLConnect_class,object):
	def __init__ (self, PIUMet_PCSFout, PCSF_out_list, result_path, w,beta, mu,R):
		#-----------------------
		...
		self.result_2_textfile(result_path,str(w),str(beta), str(mu), str(R))		
	#--------------------------------
	...
	#-----------------------------------
	def result_2_textfile(self,result_path,w,beta, mu,R):
		print (result_path)
		if result_path[-1:]=="/":
			f_net=open(result_path+("result_union_net_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
			f_node_frq=open(result_path+("result_node_frequency_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
			f_edge_frq=open(result_path+("result_edge_frequency_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
		else:
			f_net=open(result_path+"/"+("result_union_net_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
			f_node_frq=open(result_path+"/"+("result_node_frequency_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
			f_edge_frq=open(result_path+"/"+("result_edge_frequency_w%s_b%s_mu%s_R%s.txt" %(w,beta, mu,R)),'w')
