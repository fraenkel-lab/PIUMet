...
class resulting_union_net_class(MySQLConnect_class,object):
	...
	def Metabolite_MCPid_commonName(self,node):
		...
		col_MCPid='Met_ID'
		col_name='Common_Name'
		col_name_synonym='Synonyms'
		sql_select="select %s, %s from %s" %(col_name, col_name_synonym, table)
	...
	def result_2_textfile(self,result_path):
		...
		for node in self.union_net.nodes():
			...
			f_node_frq.write(node_name+"\t"+self.union_net.node[node]['frequency']+"\n")
