...
class resulting_union_net_class(MySQLConnect_class,object):
	...
	def result_2_textfile(self,result_path,parameters_dic):
		print (result_path)
		if result_path[-1:]=="/":
			f_net=open(result_path+("result_union_net_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
			f_node_frq=open(result_path+("result_node_frequency_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
			f_edge_frq=open(result_path+("result_edge_frequency_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
		else:
			f_net=open(result_path+"/"+("result_union_net_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
			f_node_frq=open(result_path+"/"+("result_node_frequency_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
			f_edge_frq=open(result_path+"/"+("result_edge_frequency_w%s_b%s_mu%s.txt" %(parameters_dic['w'],parameters_dic['b'],parameters_dic['mu'])),'w')
