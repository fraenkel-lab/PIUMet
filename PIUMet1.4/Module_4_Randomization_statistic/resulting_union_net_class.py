...
class resulting_union_net_class(MySQLConnect_class, object):
    def __init__(self, PIUMet_PCSFout, PCSF_out_list, result_path, parameters_dic):
        # -----------------------
        ...
        # -----------------------
        # 2. calculating the frequency of nodes and edges in the resulting nets, save as node and edge attribute
        PCSF_out_list.append(PIUMet_PCSFout)
        print ("Calculating the statistics")
        self.node_edge_frq_calc(PCSF_out_list)
        # -----------------------
        # 3.converting the MCP IDs to common name, and add as node attribute
        self.ID_conversion_MCPid_2_commonName()
        # -----------------------
        # 4. writing the results as text files
        print ("writing the resulting text files at the follwoing directory:")
        self.result_2_textfile(result_path, parameters_dic)
    ...
    # -----------------------------------
    def result_2_textfile(self, result_path, parameters_dic):
        print (result_path)
        if result_path[-1:] == "/":
            f_net = open(result_path + (
                "result_union_net_w%s_b%s_mu%s.txt" % (parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])),
                         'w')
            f_node_frq = open(result_path + ("result_node_frequency_w%s_b%s_mu%s.txt" % (
                parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])), 'w')
            f_edge_frq = open(result_path + ("result_edge_frequency_w%s_b%s_mu%s.txt" % (
                parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])), 'w')
        else:
            f_net = open(result_path + "/" + (
                "result_union_net_w%s_b%s_mu%s.txt" % (parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])),
        ...
        for node in self.union_net.nodes():
            if node != 'DUMMY':
                ...
                if self.union_net.node[node].has_key('HMDB_ID'):
                    ...
                    node_HMDBid = " "
                f_node_frq.write(
                    node_name + "\t" + self.union_net.node[node]['frequency'] + "\t" + self.union_net.node[node][
                        'type'] + "\t" + node_HMDBid + "\n")
                # print (node_name+"\t"+self.union_net.node[node]['frequency'])
        f_node_frq.close()
        # ----------------------------------
