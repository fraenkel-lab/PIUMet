...
class PCSFInput(object):
    def __init__(self,prizeFile,edgeFile,confFile,dummyMode,knockout,garnet,shuffle,musquared,excludeT):
        ...
        try:
            ...
            processes = None
        try:
            print 'Continuing with parameters w = %f, b = %f, mu = %f, n = %f.' \
                  %(float(w), float(b), mu, n)
        except:
    ...
    def runPCSF(self, mstpath, seed):
        """
        Passes the information in this input object to msgsteiner, and returns the results.
        
        INPUT: mstpath - points to the directory where msgsteiner is held.
        ...
        RETURNS: edgeList - the contents of stdout from msgsteiner: a list of edges in the 
                 ...
                 info - the contents of stderr from msgsteiner: if all goes well, a report on
                             the optimization
        """
        print 'Preparing information to send to the MST algorithm...\n'
        ...
        if errcode:
            ...
            sys.exit('ERROR: There was a problem running the MST algorithm. <%s>: %s' \
                     %(errcode, errmess))
        print 'MST run finished with the parameters: w = %s, b = %s, mu = %s' \
              '\n' \
              %(self.w, self.b, self.mu)
        input.close()
    ...
    def __init__(self, inputObj, edgeList, info, outputpath, outputlabel, betweenness):
        ...
        for edge in edges:
            ...
            if len(words) == 2:
                ...
                if words[0] == 'DUMMY':
                    sys.exit('ERROR: Tree returned from MST algorithm has incorrect '\
                ...
                try:
                    ...
                    #edge not found in either dictionary
                        ...
                        sys.exit('ERROR: Edges were returned from the MST algorithm '\
...
def PCSF(inputObj,mstpath,seed):
    (Edge, Info) = inputObj.runPCSF(mstpath,seed)
    return (Edge, Info)

def changeValuesAndMergeResults(func, seed, inputObj, numRuns, mstpath, outputpath, outputlabel,
                                excludeT):
    """
    Changes the prizes/edges in the PCSFInput object according to func and runs the MST 
    ...
    INPUT: func - the function which takes inputObj and changes the prize/edge values 
           ...
           mstpath - path to the directory where msgsteiner is kept
           outputpath - path to the directory where output files should be stored
           outputlabel - a label with which to name all of the output files for this run
    ...
    if seed != None:
        ...
        results = [pool.apply_async(PCSF, args=(func(inputObj, seed+i, excludeT),
                  mstpath, seed+i,)) for i in xrange(numRuns)]
    else:
        results = [pool.apply_async(PCSF, args=(func(inputObj, seed, excludeT),
                  mstpath,seed,)) for i in xrange(numRuns)]
...
def crossValidation(k, rep, PCSFInputObj, seed, mstpath, outputpath, outputlabel):
    """
    ...
    INPUT: k - the number of "folds" to seperate the prize nodes into.
           ...
           seed - number to give to the random number generator
           mstpath - path to msgsteiner code
           outputpath - path to the directory where output files should be stored
           outputlabel - a label with which to name all of the output files for this run
    ...
    for i in range(0,k):
        ...
        (newEdgeList, newInfo) = newPCSFInputObj.runPCSF(mstpath, seed)
...
def main():
    ...
    parser.add_argument("--mstpath", dest='mstpath',  help='Full path to the message passing code. '\
    ...
    try:
        with open(options.mstpath): pass
    except IOError:
        sys.exit('ERROR: The msgsteiner code was not found in the correct directory. '\
                 'Please use --mstpath to specify the path to the msgsteiner code.')
    ...
    (edgeList, info) = inputObj.runPCSF(options.mstpath, options.seed)
    ...
    if options.noiseNum > 0:
        merged = changeValuesAndMergeResults(noiseEdges, options.seed, inputObj, 
                                             options.noiseNum, options.mstpath, 
    ...
    if options.shuffleNum > 0:
        merged = changeValuesAndMergeResults(shufflePrizes, options.seed, inputObj, 
                                             options.shuffleNum, options.mstpath, 
                                             options.outputpath, options.outputlabel,
                                             options.excludeT)
    ...
    if options.termNum > 0:
        merged = changeValuesAndMergeResults(randomTerminals,options.seed, inputObj,
                                             options.termNum, options.mstpath,
                                             options.outputpath, options.outputlabel,
                                             options.excludeT)
    ...
    if options.cv != None:
        if options.cv_reps == None:
            crossValidation(options.cv, 1, inputObj, options.seed, options.mstpath, \
                            options.outputpath, options.outputlabel)
        else:
            for i in range(0,options.cv_reps):
                crossValidation(options.cv, i+1, inputObj, options.seed, options.mstpath, \
                            options.outputpath, options.outputlabel)
