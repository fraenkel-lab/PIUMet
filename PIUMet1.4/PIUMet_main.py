...
def writing_output_summary_file(result_path, parameters_dic, PIUMet_input, PIUMet_union_result):
    ...
    if result_path[:-1] == "/":
        f = open(result_path + (
        "result_summary_w%s_b%s_mu%s.txt" % (parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])), 'w')
    else:
        f = open(result_path + "/" + (
        "result_summary_w%s_b%s_mu%s.txt" % (parameters_dic['w'], parameters_dic['b'], parameters_dic['mu'])), 'w')
...
def main():
    ...
    parser.add_option("-p", "--parameteres",
                      help="a file containign the paramters of the model, including w,b,D,mu,mzW,Da as 'parameteres_name = parametere_value'",
                      default="/home/lpirhaji/ADomics_projects/data/parameteres_w25mu015.txt")
